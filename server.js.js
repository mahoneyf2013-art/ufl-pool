...
// AUTH — flexible: supports both header AND ?token= query param (for CSV downloads)
function authFlexible(rq,rs,nx){
  const h=rq.headers.authorization;
  const qt=rq.query.token;
  const raw=h?h.split(" ")[1]:qt;
  if(!raw)return rs.status(401).json({error:"No token"});
  try{rq.user=jwt.verify(raw,JWT_SECRET);nx();}catch{rs.status(401).json({error:"Invalid token"});}
}
  ...
  try{
    ...
    for(const ev of(data.events||[])){
      ...
      const match=ourGames.find(g=>{
        const nmMatch=g.home_team===hmName||g.away_team===awName||g.home_team?.includes(hm.team?.abbreviation)||g.away_team?.includes(aw.team?.abbreviation);
...
function gradeOneLeg(b){
  ...
  const ts=hs+as;let r="loss";
  if(b.bet_type==="spread"){const ps=b.pick===ht?hs:as,os=b.pick===ht?as:hs,m=ps-os+b.line;r=m>0?"win":m===0?"push":"loss";}
  else if(b.bet_type==="over")r=ts>b.line?"win":ts===b.line?"push":"loss";
  else if(b.bet_type==="under")r=ts<b.line?"win":ts===b.line?"push":"loss";
  else if(b.bet_type==="moneyline"){if(hs===as)r="push";else r=b.pick===(hs>as?ht:at)?"win":"loss";}
  return r;
}
...
async function gradeBets(){
  ...
  for(const{parlay_group:pg}of pgs){
    ...
    for(const l of legs){if(l.result==="pending"&&l.gs==="final"){const lr=gradeOneLeg(l);await pool.query("UPDATE bets SET result=$1 WHERE id=$2",[lr,l.id]);l.result=lr;}}
    const hasLoss=legs.some(l=>l.result==="loss");
    if(hasLoss){for(const l of legs){if(l.result==="pending")await pool.query("UPDATE bets SET result='loss',payout=0 WHERE id=$1",[l.id]);}console.log("Parlay "+pg+" lost");continue;}
    ...
    if(allWon||hasPush){
      let mt=1;
      for(const l of legs){if(l.result==="win"){const o=l.odds||-110;mt*=o<0?1+100/Math.abs(o):1+o/100;}}
...
rs.json({success:true,betId:id,newBalance:shouldDeduct?m.balance-wager:m.balance});}catch(e){console.error(e);rs.status(500).json({error:"Server error"});}});
...
(SELECT COALESCE(SUM(wager),0) FROM (SELECT DISTINCT ON(COALESCE(parlay_group,id)) wager FROM bets WHERE member_id=pm.id AND result='pending') sub)as pending_amount,
(pm.balance+(SELECT COALESCE(SUM(wager),0) FROM (SELECT DISTINCT ON(COALESCE(parlay_group,id)) wager FROM bets WHERE member_id=pm.id AND result='pending') sub2))as display_balance
FROM pool_members pm JOIN users u ON pm.user_id=u.id WHERE pm.pool_id=$1 AND pm.status='active'
ORDER BY(pm.balance+(SELECT COALESCE(SUM(wager),0) FROM (SELECT DISTINCT ON(COALESCE(parlay_group,id)) wager FROM bets WHERE member_id=pm.id AND result='pending') sub3))DESC`,[rq.params.poolId]));}catch(e){console.error(e);rs.status(500).json({error:"Server error"});}});

// ═══ ESPN LIVE DETAIL ═══
async function fetchESPNLiveDetail(){
  try{
    const r=await fetch("https://site.api.espn.com/apis/site/v2/sports/football/ufl/scoreboard");
    if(!r.ok)return{};const data=await r.json();const details={};
    for(const ev of(data.events||[])){
      const c=ev.competitions?.[0];if(!c)continue;
      const hm=c.competitors?.find(x=>x.homeAway==="home");const aw=c.competitors?.find(x=>x.homeAway==="away");
      if(!hm||!aw)continue;const status=c.status||{};
      const detail={espn_id:ev.id,home_name:hm.team?.displayName,away_name:aw.team?.displayName,home_abbr:hm.team?.abbreviation,away_abbr:aw.team?.abbreviation,home_score:parseInt(hm.score||0),away_score:parseInt(aw.score||0),home_logo:hm.team?.logo,away_logo:aw.team?.logo,home_color:hm.team?.color?`#${hm.team.color}`:null,away_color:aw.team?.color?`#${aw.team.color}`:null,home_record:hm.records?.[0]?.summary,away_record:aw.records?.[0]?.summary,state:status.type?.state||"pre",completed:status.type?.completed||false,period:status.period||0,clock:status.displayClock||"",detail:status.type?.shortDetail||"",description:status.type?.detail||"",situation:null,lastPlay:null,leaders:null};
      if(c.situation){detail.situation={down:c.situation.down,distance:c.situation.distance,yardLine:c.situation.yardLine,downDistanceText:c.situation.downDistanceText||(c.situation.down?`${c.situation.down}${c.situation.down===1?"st":c.situation.down===2?"nd":c.situation.down===3?"rd":"th"} & ${c.situation.distance}`:""),possession:c.situation.possession?.id||null,possessionText:c.situation.possessionText||null,isRedZone:c.situation.isRedZone||false,homeTimeouts:c.situation.homeTimeouts,awayTimeouts:c.situation.awayTimeouts};if(c.situation.lastPlay)detail.lastPlay={text:c.situation.lastPlay.text||"",team:c.situation.lastPlay.team?.id||null};}
      if(c.leaders&&c.leaders.length>0)detail.leaders=c.leaders.map(lg=>({name:lg.name||lg.displayName||"",shortName:lg.shortDisplayName||"",athletes:(lg.leaders||[]).slice(0,2).map(a=>({name:a.athlete?.displayName||"",team:a.athlete?.team?.abbreviation||"",value:a.displayValue||a.value||""}))}));
      const homeLS=hm.linescores||[],awayLS=aw.linescores||[];
      if(homeLS.length>0||awayLS.length>0)detail.linescores={home:homeLS.map(x=>x.value),away:awayLS.map(x=>x.value)};
      details[hm.team?.displayName]=detail;details[aw.team?.displayName]=detail;
...
app.get("/api/pools/:poolId/live",auth,async(rq,rs)=>{try{
  ...
  await fetchESPNLiveScores();await gradeBets();
  const espnDetail=await fetchESPNLiveDetail();
  const games=await q(`SELECT * FROM games WHERE (commence_time::timestamptz <= NOW() AND status='upcoming') OR status='final' OR home_score IS NOT NULL ORDER BY commence_time DESC LIMIT 20`);
  const result=[];
  for(const g of games){
    const gb=await q(`SELECT b.id,b.bet_type,b.pick,b.line,b.odds,b.wager,b.result,b.payout,b.member_id,b.parlay_group,b.game_id,u.display_name,u.username,bg.commence_time,bg.home_team as bet_home_team,bg.away_team as bet_away_team FROM bets b JOIN pool_members pm ON b.member_id=pm.id JOIN users u ON pm.user_id=u.id JOIN games bg ON b.game_id=bg.id WHERE b.game_id=$1 AND b.pool_id=$2 ORDER BY b.wager DESC`,[g.id,rq.params.poolId]);
    const parlayGroups=new Set();for(const b of gb){if(b.parlay_group)parlayGroups.add(b.parlay_group);}
    let extraLegs=[];
    if(parlayGroups.size>0){const pgArr=[...parlayGroups];extraLegs=await q(`SELECT b.id,b.bet_type,b.pick,b.line,b.odds,b.wager,b.result,b.payout,b.member_id,b.parlay_group,b.game_id,u.display_name,u.username,bg.commence_time,bg.home_team as bet_home_team,bg.away_team as bet_away_team FROM bets b JOIN pool_members pm ON b.member_id=pm.id JOIN users u ON pm.user_id=u.id JOIN games bg ON b.game_id=bg.id WHERE b.parlay_group=ANY($1) AND b.game_id!=$2 AND b.pool_id=$3`,[pgArr,g.id,rq.params.poolId]);}
    const allBets=[...gb,...extraLegs].map(b=>({...b,home_team:b.bet_home_team,away_team:b.bet_away_team}));
...
// ═══ SCHEDULE ═══
app.get("/api/schedule",async(rq,rs)=>{try{let espnWeeks=[];try{const cr=await fetch("https://site.api.espn.com/apis/site/v2/sports/football/ufl/scoreboard");const cd=await cr.json();const tw=cd.leagues?.[0]?.calendar?.[0]?.entries?.length||10;for(let w=1;w<=tw;w++){try{const wr=await fetch(`https://site.api.espn.com/apis/site/v2/sports/football/ufl/scoreboard?week=${w}`);const wd=await wr.json();if(wd.events&&wd.events.length>0){const games=wd.events.map(ev=>{const c=ev.competitions?.[0],hm=c?.competitors?.find(x=>x.homeAway==="home"),aw=c?.competitors?.find(x=>x.homeAway==="away"),done=c?.status?.type?.completed||false;const broadcasts=c?.geoBroadcasts||c?.broadcasts||[];const tvNetworks=broadcasts.map(b=>b.media?.shortName||b.names?.[0]||"").filter(Boolean);const tvNetwork=tvNetworks.length>0?[...new Set(tvNetworks)].join(", "):null;return{espn_id:ev.id,name:ev.name,date:ev.date,home_team:hm?.team?.displayName,away_team:aw?.team?.displayName,home_abbr:hm?.team?.abbreviation,away_abbr:aw?.team?.abbreviation,home_score:done?parseInt(hm?.score||0):null,away_score:done?parseInt(aw?.score||0):null,home_record:hm?.records?.[0]?.summary,away_record:aw?.records?.[0]?.summary,status:done?"final":c?.status?.type?.name||"upcoming",status_detail:c?.status?.type?.shortDetail||"",venue:c?.venue?.fullName,tv:tvNetwork,home_logo:hm?.team?.logo,away_logo:aw?.team?.logo,home_color:hm?.team?.color?`#${hm.team.color}`:null,away_color:aw?.team?.color?`#${aw.team.color}`:null};});espnWeeks.push({week:w,games});}}catch(e){}}}catch(e){console.error("ESPN fail:",e.message);}if(espnWeeks.length>0){const our=await q("SELECT * FROM games ORDER BY commence_time");for(const wk of espnWeeks){for(const g of wk.games){const m=our.find(x=>{const nm=x.home_team===g.home_team||x.away_team===g.away_team||x.home_team?.includes(g.home_abbr)||x.away_team?.includes(g.away_abbr);const dt=Math.abs(new Date(x.commence_time)-new Date(g.date))<172800000;return nm&&dt;});if(m){g.odds_game_id=m.id;g.spread_home=m.spread_home;g.spread_away=m.spread_away;g.total=m.total;g.moneyline_home=m.moneyline_home;g.moneyline_away=m.moneyline_away;await pool.query("UPDATE games SET week=$1 WHERE id=$2",[wk.week,m.id]);}}}return rs.json(espnWeeks);}const games=await q("SELECT * FROM games ORDER BY commence_time ASC");const weeks={};for(const g of games){const w=g.week||1;if(!weeks[w])weeks[w]={week:w,games:[]};weeks[w].games.push({...g,date:g.commence_time,status_detail:g.status});}rs.json(Object.values(weeks).sort((a,b)=>a.week-b.week));}catch(e){console.error(e);rs.status(500).json({error:"Server error"});}});
...
app.get("/api/espn/summary/:eventId",async(rq,rs)=>{try{const r=await fetch(`https://site.api.espn.com/apis/site/v2/sports/football/ufl/summary?event=${rq.params.eventId}`);if(!r.ok)return rs.status(502).json({error:"ESPN unavailable"});const data=await r.json();const normalized={scoringPlays:data.scoringPlays||[],boxscore:data.boxscore||{teams:[],players:[]},header:data.header||null,drives:data.drives||null,leaders:data.leaders||[],gameInfo:data.gameInfo||null,keyEvents:data.keyEvents||[],article:data.article||null};if(!normalized.boxscore.teams)normalized.boxscore.teams=[];if(!normalized.boxscore.players)normalized.boxscore.players=[];rs.json(normalized);}catch(e){rs.status(502).json({error:"ESPN unavailable"});}});

app.get("/api/espn/summary-by-team/:abbr",async(rq,rs)=>{try{const scr=await fetch("https://site.api.espn.com/apis/site/v2/sports/football/ufl/scoreboard");if(!scr.ok)return rs.status(502).json({error:"ESPN unavailable"});const scd=await scr.json();const abbr=rq.params.abbr.toUpperCase();const ev=(scd.events||[]).find(e=>{const c=e.competitions?.[0];if(!c)return false;return c.competitors?.some(x=>x.team?.abbreviation?.toUpperCase()===abbr);});if(!ev)return rs.status(404).json({error:"No game found for "+abbr});const r=await fetch(`https://site.api.espn.com/apis/site/v2/sports/football/ufl/summary?event=${ev.id}`);if(!r.ok)return rs.status(502).json({error:"ESPN unavailable"});const data=await r.json();const normalized={scoringPlays:data.scoringPlays||[],boxscore:data.boxscore||{teams:[],players:[]},header:data.header||null,leaders:data.leaders||[],gameInfo:data.gameInfo||null,keyEvents:data.keyEvents||[]};if(!normalized.boxscore.teams)normalized.boxscore.teams=[];if(!normalized.boxscore.players)normalized.boxscore.players=[];rs.json(normalized);}catch(e){rs.status(502).json({error:"ESPN unavailable"});}});
...
app.get("/api/pools/:poolId/profile/:username",auth,async(rq,rs)=>{try{const viewer=await q1("SELECT * FROM pool_members WHERE pool_id=$1 AND user_id=$2 AND status='active'",[rq.params.poolId,rq.user.id]);if(!viewer)return rs.status(403).json({error:"Not in pool"});const u=await q1("SELECT id,username,display_name FROM users WHERE username=$1",[rq.params.username]);if(!u)return rs.status(404).json({error:"User not found"});const m=await q1("SELECT * FROM pool_members WHERE pool_id=$1 AND user_id=$2",[rq.params.poolId,u.id]);if(!m)return rs.status(404).json({error:"Not in this pool"});const stats=await q1(`SELECT COUNT(*)as total_bets,COUNT(*)FILTER(WHERE result='win')as wins,COUNT(*)FILTER(WHERE result='loss')as losses,COUNT(*)FILTER(WHERE result='push')as pushes,COUNT(*)FILTER(WHERE result='pending')as pending,COALESCE(SUM(wager),0)as total_wagered,COALESCE(SUM(payout),0)as total_payout,COALESCE(SUM(wager)FILTER(WHERE result='pending'),0)as pending_amount,COALESCE(SUM(CASE WHEN result='win' THEN payout-wager WHEN result='loss' THEN -wager ELSE 0 END),0)as net_profit,COALESCE(AVG(wager),0)as avg_wager FROM bets WHERE member_id=$1 AND parlay_group IS NULL`,[m.id]);const byType=await q(`SELECT bet_type,COUNT(*)as total,COUNT(*)FILTER(WHERE result='win')as wins,COUNT(*)FILTER(WHERE result='loss')as losses FROM bets WHERE member_id=$1 AND parlay_group IS NULL GROUP BY bet_type`,[m.id]);const graded=await q(`SELECT result FROM bets WHERE member_id=$1 AND result IN('win','loss')AND parlay_group IS NULL ORDER BY created_at DESC LIMIT 20`,[m.id]);let streak=0,streakType="";if(graded.length>0){streakType=graded[0].result;for(const b of graded){if(b.result===streakType)streak++;else break;}}const wr=parseInt(stats.total_bets)>0?((parseInt(stats.wins)/(parseInt(stats.wins)+parseInt(stats.losses)))*100):0;const roi=parseInt(stats.total_wagered)>0?((parseInt(stats.net_profit)/parseInt(stats.total_wagered))*100):0;const recentBets=await q(`SELECT b.bet_type,b.pick,b.line,b.odds,b.wager,b.result,b.payout,b.created_at,g.home_team,g.away_team,g.home_score,g.away_score FROM bets b JOIN games g ON b.game_id=g.id WHERE b.member_id=$1 AND b.result!='pending' AND b.parlay_group IS NULL ORDER BY b.created_at DESC LIMIT 20`,[m.id]);rs.json({username:u.username,display_name:u.display_name,balance:m.balance,role:m.role,stats:{...stats,win_rate:Math.round(wr*10)/10,roi:Math.round(roi*10)/10},byType,streak:{count:streak,type:streakType},recentBets});}catch(e){console.error(e);rs.status(500).json({error:"Server error"});}});
...
// ═══ CSV EXPORTS (use authFlexible for ?token= support) ═══
app.get("/api/pools/:poolId/export/standings",authFlexible,async(rq,rs)=>{try{const lb=await q(`SELECT u.display_name,u.username,pm.balance,pm.role,(SELECT COUNT(*)FROM bets WHERE member_id=pm.id AND result='win')as wins,(SELECT COUNT(*)FROM bets WHERE member_id=pm.id AND result='loss')as losses,(SELECT COUNT(*)FROM bets WHERE member_id=pm.id AND result='push')as pushes,(SELECT COALESCE(SUM(wager),0) FROM (SELECT DISTINCT ON(COALESCE(parlay_group,id)) wager FROM bets WHERE member_id=pm.id AND result='pending') sub)as pending,(pm.balance+(SELECT COALESCE(SUM(wager),0) FROM (SELECT DISTINCT ON(COALESCE(parlay_group,id)) wager FROM bets WHERE member_id=pm.id AND result='pending') sub2))as display_balance FROM pool_members pm JOIN users u ON pm.user_id=u.id WHERE pm.pool_id=$1 AND pm.status='active' ORDER BY display_balance DESC`,[rq.params.poolId]);let csv="Rank,Name,Username,Balance,Display Balance,Wins,Losses,Pushes,Pending,Role\n";lb.forEach((p,i)=>{csv+=`${i+1},"${p.display_name}",${p.username},${p.balance},${p.display_balance},${p.wins},${p.losses},${p.pushes},${p.pending},${p.role}\n`;});rs.setHeader("Content-Type","text/csv");rs.setHeader("Content-Disposition","attachment; filename=standings.csv");rs.send(csv);}catch(e){rs.status(500).json({error:"Server error"});}});

app.get("/api/pools/:poolId/export/bets",authFlexible,async(rq,rs)=>{try{const m=await q1("SELECT * FROM pool_members WHERE pool_id=$1 AND user_id=$2",[rq.params.poolId,rq.user.id]);if(!m)return rs.status(403).json({error:"Not in pool"});const bets=await q(`SELECT b.*,g.home_team,g.away_team,g.home_score,g.away_score,g.week,g.commence_time FROM bets b JOIN games g ON b.game_id=g.id WHERE b.member_id=$1 ORDER BY b.created_at DESC`,[m.id]);let csv="Date,Week,Game,Type,Pick,Line,Odds,Wager,Result,Payout,Profit,Parlay Group\n";for(const b of bets){const pr=b.result==="win"?b.payout-b.wager:b.result==="loss"?-b.wager:0;csv+=`"${new Date(b.created_at).toLocaleDateString()}",${b.week||""},"${b.away_team} @ ${b.home_team}",${b.bet_type},"${b.pick}",${b.line||""},${b.odds||""},${b.wager},${b.result},${b.payout},${pr},${b.parlay_group||""}\n`;}rs.setHeader("Content-Type","text/csv");rs.setHeader("Content-Disposition","attachment; filename=my-bets.csv");rs.send(csv);}catch(e){rs.status(500).json({error:"Server error"});}});

// ═══ ADMIN: EXPORT ALL ACTIVITY (for balance correction) ═══
app.get("/api/pools/:poolId/export/all-activity",authFlexible,async(rq,rs)=>{try{
  const a=await q1("SELECT * FROM pool_members WHERE pool_id=$1 AND user_id=$2 AND role='admin' AND status='active'",[rq.params.poolId,rq.user.id]);
  if(!a)return rs.status(403).json({error:"Admin required"});
  const bets=await q(`SELECT b.*,g.home_team,g.away_team,g.home_score,g.away_score,g.week,g.commence_time,g.status as game_status,u.display_name,u.username,pm.balance as current_balance FROM bets b JOIN games g ON b.game_id=g.id JOIN pool_members pm ON b.member_id=pm.id JOIN users u ON pm.user_id=u.id WHERE b.pool_id=$1 ORDER BY b.created_at ASC`,[rq.params.poolId]);
  const pl=await q1("SELECT * FROM pools WHERE id=$1",[rq.params.poolId]);
  const startBal=pl?pl.starting_balance:1000;
  let csv="Date,Username,Display Name,Bet Type,Pick,Line,Odds,Wager,Result,Payout,Profit,Parlay Group,Game,Week,Game Status,Home Score,Away Score,Current Balance,Bet ID,Member ID\n";
  for(const b of bets){const pr=b.result==="win"?(b.payout-b.wager):b.result==="loss"?(-b.wager):0;csv+=`"${new Date(b.created_at).toISOString()}","${b.username}","${b.display_name}",${b.bet_type},"${b.pick}",${b.line||""},${b.odds||""},${b.wager},${b.result},${b.payout||0},${pr},${b.parlay_group||""},"${b.away_team} @ ${b.home_team}",${b.week||""},${b.game_status},${b.home_score!=null?b.home_score:""},${b.away_score!=null?b.away_score:""},${b.current_balance},${b.id},${b.member_id}\n`;}
  csv+="\n\nMEMBER SUMMARY\nUsername,Display Name,Current Balance,Starting Balance,Expected Balance,Discrepancy,Total Wagered (Straight),Total Wagered (Parlay - Unique),Straight Wins,Straight Losses,Straight Pushes,Straight Payout,Parlay Groups,Parlay Payout\n";
  const mems=await q(`SELECT pm.*,u.username,u.display_name FROM pool_members pm JOIN users u ON pm.user_id=u.id WHERE pm.pool_id=$1 AND pm.status='active'`,[rq.params.poolId]);
  for(const m of mems){
    const mb=bets.filter(b=>b.member_id===m.id);
    const straight=mb.filter(b=>!b.parlay_group);
    const straightWagered=straight.reduce((s,b)=>s+b.wager,0);
    const straightPayout=straight.filter(b=>b.result==="win"||b.result==="push").reduce((s,b)=>s+(b.payout||0),0);
    const straightWins=straight.filter(b=>b.result==="win").length;
    const straightLosses=straight.filter(b=>b.result==="loss").length;
    const straightPushes=straight.filter(b=>b.result==="push").length;
    const parlayGroups=[...new Set(mb.filter(b=>b.parlay_group).map(b=>b.parlay_group))];
    let parlayWagered=0,parlayPayout=0;
    for(const pg of parlayGroups){const legs=mb.filter(b=>b.parlay_group===pg);if(legs.length>0){parlayWagered+=legs[0].wager;const payoutLeg=legs.find(l=>l.payout>0);if(payoutLeg)parlayPayout+=payoutLeg.payout;}}
    const expectedBal=startBal-straightWagered-parlayWagered+straightPayout+parlayPayout;
    const discrepancy=m.balance-expectedBal;
    csv+=`"${m.username}","${m.display_name}",${m.balance},${startBal},${expectedBal},${discrepancy},${straightWagered},${parlayWagered},${straightWins},${straightLosses},${straightPushes},${straightPayout},${parlayGroups.length},${parlayPayout}\n`;
  }
  rs.setHeader("Content-Type","text/csv");rs.setHeader("Content-Disposition",`attachment; filename=all-activity-${rq.params.poolId}.csv`);rs.send(csv);
}catch(e){console.error(e);rs.status(500).json({error:"Server error"});}});
...
// ═══ CRON ═══
cron.schedule("*/2 18-23 * * 5,6",()=>{console.log("Cron:ESPN live (Fri/Sat)");fetchESPNLiveScores();});
cron.schedule("*/2 12-23 * * 0",()=>{console.log("Cron:ESPN live (Sun)");fetchESPNLiveScores();});
...
// ═══ START ═══
app.listen(PORT,async()=>{console.log("UFL Pool v2.0 on port "+PORT);await initDB();if(ODDS_API_KEY){console.log("Fetching odds...");const c=await fetchOdds();console.log("Loaded "+c+" games");}});
